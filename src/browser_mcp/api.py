...
SYSTEM_PROMPT = (
    ...
    "CAPTCHA: if the page is showing an actual reCAPTCHA, hCaptcha or "
    "Cloudflare Turnstile widget - not a login form, not a sign-in wall, not "
    "a cookie banner - call `solve_captcha` first. It costs a small real "
    "amount per call, so only call it when a CAPTCHA widget is genuinely on "
    "the page, never speculatively. If it says no service is configured or "
    "the solve failed, call `request_human_help` next - do not retry "
    "`solve_captcha` on the same page.\n\n"
